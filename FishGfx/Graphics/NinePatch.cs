	...
	internal static class NinePatchTessellator {
		internal static Vertex2[] Create(Vector2 position, Vector2 size, Vector2 textureSize, NinePatchInsets insets, Color color) {
			...
			if (insets.Top + insets.Bottom > textureSize.Y)
				throw new ArgumentOutOfRangeException(nameof(insets), "Vertical insets exceed the texture height.");
			if (size.X == 0 || size.Y == 0)
				return Array.Empty<Vertex2>();

			(float left, float right) = FitBorders(insets.Left, insets.Right, size.X);
			(float bottom, float top) = FitBorders(insets.Bottom, insets.Top, size.Y);
			float[] x = { position.X, position.X + left, position.X + size.X - right, position.X + size.X };
			float[] y = { position.Y, position.Y + bottom, position.Y + size.Y - top, position.Y + size.Y };
			float[] u = { 0, insets.Left / textureSize.X, 1 - insets.Right / textureSize.X, 1 };
			float[] v = { 0, insets.Bottom / textureSize.Y, 1 - insets.Top / textureSize.Y, 1 };
		...
		private static void EmitQuad(Vertex2[] vertices, int offset, float x0, float y0, float x1, float y1,
			...
			vertices[offset + 3] = new Vertex2(new Vector2(x0, y0), new Vector2(u0, v0), color);
			vertices[offset + 4] = new Vertex2(new Vector2(x1, y0), new Vector2(u1, v0), color);
