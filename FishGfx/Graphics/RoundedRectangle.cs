...
namespace FishGfx.Graphics {
	public readonly struct CornerRadii : IEquatable<CornerRadii> {
		...
		public CornerRadii(float radius) : this(radius, radius, radius, radius) {
		}
		...
		public bool Equals(CornerRadii other) => TopLeft == other.TopLeft && TopRight == other.TopRight
	...
	internal static class RoundedRectangleTessellator {
		...
		private static List<Vector2> Boundary(Vector2 position, Vector2 size, CornerRadii radii, int requestedSegments) {
			...
			AppendCorner(points, new Vector2(x1 - radii.BottomRight, y0 + radii.BottomRight), radii.BottomRight, -MathF.PI / 2, 0, requestedSegments, new Vector2(x1, y0));
			AppendCorner(points, new Vector2(x1 - radii.TopRight, y1 - radii.TopRight), radii.TopRight, 0, MathF.PI / 2, requestedSegments, new Vector2(x1, y1));
			AppendCorner(points, new Vector2(x0 + radii.TopLeft, y1 - radii.TopLeft), radii.TopLeft, MathF.PI / 2, MathF.PI, requestedSegments, new Vector2(x0, y1));
			AppendCorner(points, new Vector2(x0 + radii.BottomLeft, y0 + radii.BottomLeft), radii.BottomLeft, MathF.PI, MathF.PI * 1.5f, requestedSegments, new Vector2(x0, y0));
			return points;
		}

		private static void AppendCorner(List<Vector2> points, Vector2 center, float radius, float start, float end,
			int requestedSegments, Vector2 sharpCorner) {
			if (radius == 0) {
		...
		private static void ValidateSize(Vector2 size) {
			if (!float.IsFinite(size.X) || !float.IsFinite(size.Y) || size.X < 0 || size.Y < 0)
				throw new ArgumentOutOfRangeException(nameof(size), "Rounded rectangle size must be finite and non-negative.");
		}
	}
}
