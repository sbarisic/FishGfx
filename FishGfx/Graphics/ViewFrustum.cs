...
namespace FishGfx.Graphics
{
	...
	public readonly struct ViewFrustum
	{
		...
		private static bool IsInside(Plane plane, AABB bounds)
		{
			Vector3 positive = new Vector3(
				plane.Normal.X >= 0 ? bounds.Maxs.X : bounds.Mins.X,
				plane.Normal.Y >= 0 ? bounds.Maxs.Y : bounds.Mins.Y,
				plane.Normal.Z >= 0 ? bounds.Maxs.Z : bounds.Mins.Z
			);
