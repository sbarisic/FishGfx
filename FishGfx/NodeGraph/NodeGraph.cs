...
namespace FishGfx.NodeGraph
{
	...
	public sealed class NodeBodyValue
	{
		...
		internal NodeBodyValue(NodeParameterDescriptor descriptor)
		{
			...
			object initial =
				descriptor.Parameter.HasDefaultValue && descriptor.Parameter.DefaultValue != DBNull.Value
	...
	public sealed class FunctionNode
	{
		...
		internal FunctionNode(NodeFunctionDescriptor function, Vector2 position, Guid? id = null)
		{
			...
			{
				if (parameter.IsBody)
					BodyValues.Add(new NodeBodyValue(parameter));
				else
		...
		private NodePort NewPort(string name, Type type, NodePortDirection direction)
		{
	...
	public sealed class FunctionNodeGraph
	{
		...
		public FunctionNode CreateNode(NodeFunctionDescriptor descriptor, Vector2 position)
		{
		...
		internal FunctionNode CreateNode(NodeFunctionDescriptor descriptor, Vector2 position, Guid id)
		{
			FunctionNode node = new FunctionNode(
				descriptor ?? throw new ArgumentNullException(nameof(descriptor)),
				position,
		...
		public void Remove(FunctionNode node)
		{
		...
		public void Remove(NodeConnection connection)
		{
	...
	public sealed class NodeEvaluationResult
	{
		...
		public string Summary =>
			Success ? $"Evaluated {SuccessfulNodes} nodes" : $"{FailedNodes} errors, {SuccessfulNodes} nodes evaluated";
	}
	...
	public sealed class FunctionNodeEvaluator
	{
		...
		public NodeEvaluationResult Evaluate(FunctionNodeGraph graph)
		{
			...
			{
				SuccessfulNodes = graph.Nodes.Count(n => n.EvaluationState == NodeEvaluationState.Success),
				FailedNodes = graph.Nodes.Count(n =>
					n.EvaluationState == NodeEvaluationState.Error || n.EvaluationState == NodeEvaluationState.Skipped
				),
			};
		}
		...
		private static void Invoke(FunctionNode node, FunctionNodeGraph graph)
		{
			try
			{
				object[] args = new object[node.Function.Parameters.Count];
				int inputIndex = 0,
					bodyIndex = 0;
				for (int i = 0; i < args.Length; i++)
				{
					...
					{
						NodePort input = node.Inputs[inputIndex++];
						NodeConnection connection = graph.ConnectionAtInput(input);
			...
			{
				Fail(node, ex.InnerException?.Message ?? ex.Message, NodeEvaluationState.Error);
			}
			catch (Exception ex)
			{
				Fail(node, ex.Message, NodeEvaluationState.Error);
			}
		}
