...
namespace FishGfx.SmokeTest {
	internal static class PrimitiveScenes {
		...
		private static void DrawRoundedRectangles(float time, Texture _) {
			for (int row = 0; row < 4; row++)
				for (int column = 0; column < 4; column++) {
					...
					Gfx.RoundedRectangle(x, y, 280, 150, radii, 3 + row * 2,
		...
		private static void DrawFilledRoundedRectangles(float time, Texture _) {
			for (int row = 0; row < 3; row++)
				for (int column = 0; column < 5; column++) {
					...
					Gfx.FilledRoundedRectangle(x, y, width, 190, radii,
		...
		private static void DrawTexturedRoundedRectangles(float time, Texture texture) {
			for (int row = 0; row < 3; row++)
				for (int column = 0; column < 4; column++) {
					...
					Gfx.TexturedRoundedRectangle(x, y, 300, height, radii, texture,
		...
		private static void DrawTexturedCircles(float time, Texture texture) {
			for (int ring = 0; ring < 4; ring++) {
				...
				for (int i = 0; i < count; i++) {
					...
					Gfx.TexturedCircle(center, 35 + ring * 10, texture,
		...
		private static void DrawTexturedEllipses(float time, Texture texture) {
			for (int row = 0; row < 3; row++)
				for (int column = 0; column < 4; column++) {
					...
					Gfx.TexturedEllipse(center, radii, texture,
		...
		private static void DrawNinePatches(float time, Texture texture) {
			...
			Gfx.NinePatch(1080, 190, animatedWidth, animatedHeight, texture, insets,
		...
		private static void DrawCircles(float time, Texture _) {
			for (int row = 0; row < 3; row++)
				for (int column = 0; column < 6; column++) {
					...
					Gfx.Circle(center, radius, 3 + column * 2,
		...
		private static void DrawFilledCircles(float time, Texture _) {
			for (int ring = 0; ring < 4; ring++) {
				...
				for (int i = 0; i < count; i++) {
					...
					Gfx.FilledCircle(center, 24 + ring * 11,
		...
		private static void DrawRings(float time, Texture _) {
			...
			for (int i = 0; i < 5; i++)
				Gfx.Ring(fullCenter, 45 + i * 38, 70 + i * 38,
			...
			Gfx.Ring(new Vector2(1250, 650), 85, 220, -MathF.PI / 4, -MathF.PI / 4 + animatedSweep,
				new Color(80, 175, 255, 220));

			Gfx.Ring(new Vector2(1180, 260), 0, 150, MathF.PI * 1.5f, MathF.PI * 2.5f,
				new Color(255, 145, 80, 215), 6);
			Gfx.Ring(new Vector2(1600, 300), 125, 145, 0, MathF.PI * 1.35f,
		...
		private static void DrawRingLines(float time, Texture _) {
			...
			for (int i = 0; i < 5; i++)
				Gfx.RingLines(fullCenter, 35 + i * 42, 70 + i * 42, 3 + i * 2,
					new Color((byte)(75 + i * 32), (byte)(220 - i * 16), (byte)(125 + i * 23)));

			float start = time * 0.45f;
			float sweep = MathF.PI * 1.45f;
			Gfx.RingLines(new Vector2(1280, 660), 90, 230, start, start + sweep, 12,
				new Color(90, 190, 255), 0);

			Gfx.RingLines(new Vector2(1210, 250), 0, 145, MathF.PI, MathF.PI * 1.8f, 7,
				new Color(255, 170, 75), 5);
			Gfx.RingLines(new Vector2(1620, 290), 130, 130, 0, MathF.Tau, 10,
		...
		private static void DrawEllipses(float time, Texture _) {
			for (int row = 0; row < 3; row++)
				for (int column = 0; column < 5; column++) {
					...
					Gfx.Ellipse(center, radii, 4 + row * 4,
		...
		private static void DrawFilledEllipses(float time, Texture _) {
			for (int i = 0; i < 12; i++) {
				...
				Gfx.FilledEllipse(center, radii,
		...
		private static void DrawQuadraticBeziers(float time, Texture _) {
			for (int i = 0; i < 10; i++) {
				...
				Gfx.QuadraticBezier(start, control, end, 3 + i,
		...
		private static void DrawCubicBeziers(float time, Texture _) {
			for (int i = 0; i < 9; i++) {
				...
				Gfx.CubicBezier(
					...
					new Vector2(1810, y),
					4 + i, new Color((byte)(75 + i * 18), (byte)(115 + i * 12), (byte)(245 - i * 12)),
					i == 0 ? 12 : 0);
			}
		}
	}
