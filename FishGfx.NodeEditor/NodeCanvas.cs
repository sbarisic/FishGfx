...
internal static class NodeGeometry
{
	...
	internal static float HeightOf(FunctionNode node) =>
		66 + (PortRowCount(node) + node.InlineValues.Count) * RowHeight + (node.Outputs.Count > 0 ? 25 : 0);
	...
	internal static Vector2 PortPosition(NodePort port)
	{
		int index =
			port.Direction == NodePortDirection.Input
				? IndexOf(port.Node.Inputs, port)
				: IndexOf(port.Node.Outputs, port);
	...
	private static int IndexOf(System.Collections.Generic.IReadOnlyList<NodePort> ports, NodePort port)
	{
