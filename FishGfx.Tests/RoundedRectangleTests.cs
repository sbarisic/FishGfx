...
public class RoundedRectangleTests {
	...
	public void OutlineIsClosedAndUsesExplicitSegmentsPerCorner() {
		Vector2[] vertices = RoundedRectangleTessellator.Outline(Vector2.Zero, new Vector2(200, 100), new CornerRadii(20), 2);
	...
	public void AsymmetricRadiiProduceExpectedCornerEndpoints() {
		Vector2[] vertices = RoundedRectangleTessellator.Outline(new Vector2(10, 20), new Vector2(300, 200),
			new CornerRadii(10, 20, 30, 40), 1);
	...
	public void CssStyleClampingPreservesRadiusProportions() {
		...
		CornerRadii asymmetric = RoundedRectangleTessellator.Clamp(new Vector2(100, 80), new CornerRadii(80, 40, 20, 20));
		Assert.Equal(new CornerRadii(64, 32, 16, 16), asymmetric);
	}

	[Fact]
	public void ZeroRadiiProduceSharpCorners() {
		Vector2[] vertices = RoundedRectangleTessellator.Outline(new Vector2(10, 20), new Vector2(100, 50), new CornerRadii(0), 0);
		Assert.Equal(new[] {
			new Vector2(110, 20), new Vector2(110, 70), new Vector2(10, 70), new Vector2(10, 20), new Vector2(110, 20)
		}, vertices);
	}

	[Fact]
	public void FilledFanHasPositiveWindingAndExpectedCount() {
		Vector2[] vertices = RoundedRectangleTessellator.Filled(Vector2.Zero, new Vector2(200, 100), new CornerRadii(20), 2);
	...
	public void PlanarTextureMappingCoversBoundsAndPropagatesTint() {
		...
		Vertex2[] vertices = PrimitiveTessellator.TextureVertices(positions, min, size,
			new Vector2(0.2f, 0.3f), new Vector2(0.8f, 0.9f), tint);
	...
	public void ZeroSizedShapesProduceNoGeometry() {
		...
		Assert.Empty(PrimitiveTessellator.TextureVertices(Array.Empty<Vector2>(), Vector2.Zero, Vector2.Zero,
	...
	public void InvalidInputsThrow() {
		...
		Assert.Throws<ArgumentOutOfRangeException>(() => RoundedRectangleTessellator.Outline(Vector2.Zero, new Vector2(-1, 10), new CornerRadii(1)));
		Assert.Throws<ArgumentOutOfRangeException>(() => RoundedRectangleTessellator.Outline(new Vector2(float.NaN, 0), Vector2.One, new CornerRadii(1)));
		Assert.Throws<ArgumentOutOfRangeException>(() => RoundedRectangleTessellator.Outline(Vector2.Zero, Vector2.One, new CornerRadii(1), -1));
		Assert.Throws<ArgumentOutOfRangeException>(() => PrimitiveTessellator.TextureVertices(new[] { Vector2.Zero }, Vector2.Zero, Vector2.One,
			new Vector2(1, 0), new Vector2(0, 1), Color.White));
		...
		Assert.Throws<ArgumentNullException>(() => Gfx.TexturedRoundedRectangle(0, 0, 10, 10, new CornerRadii(2), null));
	}
