...
public class RingTests {
	...
	public void SupportsArbitraryAndZeroCrossingAngles() {
		...
		Vector2[][] crossing = RingTessellator.Lines(Vector2.Zero, 10, 20,
			MathF.PI * 1.5f, MathF.PI * 2.5f, 8);
		Assert.Single(crossing);
		Assert.Equal(crossing[0][0], crossing[0][^1]);

		Vector2[][] fullFromArbitraryStart = RingTessellator.Lines(Vector2.Zero, 10, 20,
			MathF.PI * 1.5f, MathF.PI * 3.5f, 8);
	...
	public void InvalidInputsThrow() {
		...
		Assert.Throws<ArgumentOutOfRangeException>(() => RingTessellator.Filled(Vector2.Zero, 1, float.PositiveInfinity, 0, 1));
		Assert.Throws<ArgumentOutOfRangeException>(() => RingTessellator.Filled(Vector2.Zero, 1, 2, 2, 1));
		Assert.Throws<ArgumentOutOfRangeException>(() => RingTessellator.Filled(Vector2.Zero, 1, 2, 0, MathF.Tau + 0.1f));
