...
public class NinePatchTests {
	...
	public void BottomAndTopInsetsMapToCorrectVCoordinates() {
		Vertex2[] vertices = NinePatchTessellator.Create(Vector2.Zero, new Vector2(300), new Vector2(200),
			new NinePatchInsets(20, 50, 30, 40), Color.White);
	...
	public void PropagatesTintToEveryVertex() {
		...
		Vertex2[] vertices = NinePatchTessellator.Create(Vector2.Zero, new Vector2(300), new Vector2(256), Insets, tint);
		Assert.All(vertices, vertex => Assert.Equal(tint, vertex.Color));
	}

	[Fact]
	public void ZeroDestinationProducesNoGeometry() {
		Assert.Empty(NinePatchTessellator.Create(Vector2.Zero, new Vector2(0, 100), new Vector2(256), Insets, Color.White));
		Assert.Empty(NinePatchTessellator.Create(Vector2.Zero, new Vector2(100, 0), new Vector2(256), Insets, Color.White));
	}
	...
	public void InvalidInputsThrow() {
		...
		Assert.Throws<ArgumentOutOfRangeException>(() => NinePatchTessellator.Create(Vector2.Zero, new Vector2(-1, 20), new Vector2(256), Insets, Color.White));
		Assert.Throws<ArgumentOutOfRangeException>(() => NinePatchTessellator.Create(new Vector2(float.NaN, 0), Vector2.One, new Vector2(256), Insets, Color.White));
		Assert.Throws<ArgumentOutOfRangeException>(() => NinePatchTessellator.Create(Vector2.Zero, Vector2.One, new Vector2(100), Insets, Color.White));
	...
	private static (Vector2 min, Vector2 max) Bounds(Vertex2[] vertices, int patch) => Bounds(vertices.Skip(patch * 6).Take(6));
	private static (Vector2 min, Vector2 max) BoundsUv(Vertex2[] vertices, int patch) => Bounds(vertices.Skip(patch * 6).Take(6).Select(vertex => vertex.UV));
	private static (Vector2 min, Vector2 max) Bounds(IEnumerable<Vertex2> vertices) => Bounds(vertices.Select(vertex => vertex.Position));
	private static (Vector2 min, Vector2 max) Bounds(IEnumerable<Vector2> values) {
		return (new Vector2(values.Min(value => value.X), values.Min(value => value.Y)),
