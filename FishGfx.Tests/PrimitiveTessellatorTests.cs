...
public class PrimitiveTessellatorTests {
	...
	public void QuadraticBezierHasExactEndpointsAndMidpoint() {
		Vector2[] vertices = PrimitiveTessellator.QuadraticBezier(Vector2.Zero, new Vector2(10, 20), new Vector2(20, 0), 2);
	...
	public void CubicBezierHasExactEndpointsAndMidpoint() {
		Vector2[] vertices = PrimitiveTessellator.CubicBezier(Vector2.Zero, new Vector2(0, 20), new Vector2(20, 20), new Vector2(20, 0), 2);
		Assert.Equal(Vector2.Zero, vertices[0]);
	...
	public void AutomaticSegmentCountsGrowAndStayBounded() {
		...
		int smallCurve = PrimitiveTessellator.ResolveCurveSegments(0, Vector2.Zero, new Vector2(20, 0), new Vector2(40, 0));
		int largeCurve = PrimitiveTessellator.ResolveCurveSegments(0, Vector2.Zero, new Vector2(1000, 0), new Vector2(2000, 0));
	...
	public void InvalidInputsThrow() {
		Assert.Throws<ArgumentOutOfRangeException>(() => PrimitiveTessellator.EllipseOutline(Vector2.Zero, new Vector2(-1, 1)));
		Assert.Throws<ArgumentOutOfRangeException>(() => PrimitiveTessellator.EllipseOutline(new Vector2(float.NaN, 0), Vector2.One));
		Assert.Throws<ArgumentOutOfRangeException>(() => PrimitiveTessellator.EllipseOutline(Vector2.Zero, Vector2.One, 2));
		Assert.Throws<ArgumentOutOfRangeException>(() => PrimitiveTessellator.QuadraticBezier(Vector2.Zero, Vector2.One, Vector2.One, 1));
		Assert.Throws<ArgumentOutOfRangeException>(() => PrimitiveTessellator.CubicBezier(Vector2.Zero, Vector2.One, Vector2.One, new Vector2(float.PositiveInfinity, 0)));
