...
public class NodeGraphJsonTests
{
	...
	public void RoundTripPreservesGraphIdsBodyLayoutConnectionsAndView()
	{
		...
		FunctionNode source = graph.CreateNode(Find(registry, nameof(JsonFunctions.Constant)), new Vector2(12, 34));
		source.Width = 275;
		source.BodyValues[0].Text = "42";
		FunctionNode sink = graph.CreateNode(Find(registry, nameof(JsonFunctions.Identity)), new Vector2(500, 80));
		...
		Assert.Equal("42", loadedSource.BodyValues[0].Text);
	...
	{
		NodeFunctionRegistry registry = Registry();
		FunctionNodeGraph original = new FunctionNodeGraph();
		original.CreateNode(Find(registry, nameof(JsonFunctions.Constant)), Vector2.Zero);
		JsonNode root = JsonNode.Parse(NodeGraphJson.Serialize(original, new NodeGraphViewState(Vector2.Zero, 1)));
		root["version"] = 99;
		root["canvas"]["zoom"] = 99;
		root["nodes"][0]["declaringType"] = "Missing.Type";
		root["nodes"][0]["x"] = "NaN";
	...
	public void DuplicateConnectionsAndInvalidIndexesAreRejected()
	{
		NodeFunctionRegistry registry = Registry();
		FunctionNodeGraph graph = new FunctionNodeGraph();
		FunctionNode source = graph.CreateNode(Find(registry, nameof(JsonFunctions.Constant)), Vector2.Zero),
			sink = graph.CreateNode(Find(registry, nameof(JsonFunctions.Identity)), Vector2.One);
		graph.Connect(source.Outputs[0], sink.Inputs[0]);
		...
		root["connections"].AsArray().Add(root["connections"][0].DeepClone());
		root["connections"][0]["outputIndex"] = 50;
		NodeGraphLoadResult load = NodeGraphJson.Deserialize(root.ToJsonString(), registry);
		Assert.False(load.Success);
		Assert.Contains(load.Errors, e => e.Contains("port index"));
	...
	public void FileSaveReplacesAndLoadAndEvaluateProducesStructuredJson()
	{
		string directory = Path.Combine(Path.GetTempPath(), "FishGfxTests", Guid.NewGuid().ToString("N")),
			path = Path.Combine(directory, "layout.json");
		try
		{
			NodeFunctionRegistry registry = Registry();
			FunctionNodeGraph graph = new FunctionNodeGraph();
			FunctionNode node = graph.CreateNode(Find(registry, nameof(JsonFunctions.Special)), Vector2.Zero);
	...
	public void HeadlessRunnerReturnsDocumentAndMeaningfulExitCodes()
	{
		...
		try
		{
			...
			FunctionNodeGraph graph = new FunctionNodeGraph();
			graph.CreateNode(registry.Functions.First(), Vector2.Zero);
			NodeGraphJson.SaveFile(path, graph, new NodeGraphViewState(Vector2.Zero, 1));
			Console.SetOut(output);
			Assert.Equal(0, HeadlessRunner.Execute(path));
			Assert.True(JsonDocument.Parse(output.ToString()).RootElement.TryGetProperty("success", out _));
