...
string filter = (args.FirstOrDefault() ?? "").ToLowerInvariant();
if (filter == "verify-graphics") { GraphicsProbes.Verify(); return; }
if (filter.Contains("geometry")) { GraphicsProbes.Geometry(Run); return; }
if (filter == "" || filter.StartsWith("mesh") || filter.StartsWith("sort-") && !filter.StartsWith("sort-scratch") || filter.StartsWith("schedule") || filter.StartsWith("group"))
{
