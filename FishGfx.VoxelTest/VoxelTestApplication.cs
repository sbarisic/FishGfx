...
internal sealed partial class VoxelTestApplication
{
	...
	private readonly bool captureFixture;
	...
	internal VoxelTestApplication(string[] args)
	{
		...
		captureFixture = args.Contains("--capture-fixture", StringComparer.OrdinalIgnoreCase);
	...
	internal void Run()
	{
		...
		VoxelRenderer renderer = new VoxelRenderer(
			...
			new VoxelRendererOptions
			{
				...
				Meshing = new VoxelMeshingOptions { CubeMeshingMode = greedy ? VoxelCubeMeshingMode.GreedyOpaque : VoxelCubeMeshingMode.Culled },
		...
		try
		{
			while (!window.IsCloseRequested)
			{
				...
				if (captureFixture && !fixtureInstalled && streamer.IsSettled)
				{
					for (int z = 117; z < 149; z++) for (int y = 50; y < 58; y++) for (int x = 361; x < 393; x++)
						world.SetVoxel(x, y, z, new(x < 377 ? materials.Grass : materials.Bricks));
