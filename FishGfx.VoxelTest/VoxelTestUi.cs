...
internal sealed class VoxelTestUi : IDisposable
{
	...
	)
	{
		...
		statisticsLabel.Text =
			...
			+ $"lighting columns lit/promoting: {streamer.LitHorizontalCount} / {streamer.PendingLightingHorizontalCount}\n"
			...
			+ $"   mesh: {diagnostics.MeshSchedulingMilliseconds:F2} + {diagnostics.MeshUploadMilliseconds:F2} ms\n"
			+ $"mesh scheduled/uploaded/fast: {diagnostics.ScheduledMeshes} / {diagnostics.UploadedMeshes} / {diagnostics.FastCompletedMeshes}"
	...
	private void OnWindowResized(object sender, WindowResizeEventArgs args)
	{
